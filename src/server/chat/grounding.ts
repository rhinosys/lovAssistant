...
export const renderEvidence = (raw: string, sources: EvidenceSource[]): string => {
  ...
  // Swap verified images for placeholders first so the blanket URL/link guard below doesn't
  // reject the whole answer just because a legitimate ![alt](url) contains "](" and "http".
  ...
  answer = answer.replace(/ IMG(\d+) /g, (_match: string, i: string) => verifiedImages[Number(i)]);
